...
export async function suggestMoaka(query:string):Promise<{bundle:Bundle;source:'gemini';message:string}> {
 if(!process.env.GEMINI_API_KEY) throw new Error('Recipe generation is not configured')
